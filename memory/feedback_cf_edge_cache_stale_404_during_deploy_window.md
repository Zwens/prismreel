...
**🔴 2026-09-15 同日第三度重演（480筆補齊後30筆再次卡舊404）**：480/480本機檔案+commit+push全部確認齊全、VPS容器內部`curl`直打後端port驗證200，但外部live訪問仍30筆404；加`?cb=`cache-busting query string全部立即轉200，確診純CF edge cache問題非部署缺漏。使用者選擇Purge Everything（非逐一Custom Purge），執行後480/480複驗全數200，任務結案。**注意**：正確驗證路徑是後端`api.py`實際掛載的`/files/digital-characters/`（`StaticFiles`掛載名），不是`config/digital_characters/thumbnails/`原始檔案系統路徑，兩者容易搞混導致curl測試本身先踩錯路徑得到誤導性401。
