# PrismReel Model Docs And Catalog Architecture Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Build a durable model-adaptation architecture for PrismReel so official model docs can be archived, AI-friendly integration knowledge can be updated continuously, and model support in code can scale without scattered hardcoded defaults.

**Architecture:** Split the problem into three layers with strict ownership. Keep raw vendor docs in a separate archive repo as the auditable source of truth. Use Context Hub as the AI-facing knowledge layer that distills integration-critical details from those raw docs plus project-specific experience. Add a machine-readable `model_catalog` inside the PrismReel repo as the executable source of truth for model families, versions, capabilities, provider routing, UI exposure, and test coverage.
...
PrismReel is now in a zone where model churn is a product constraint, not a one-off maintenance task.
...
**Repository recommendation:** separate private repo, for example `prismreel-vendor-docs`.

**Why separate from the main PrismReel repo:**
...
**System recommendation:** Context Hub source package maintained by the PrismReel team.
...
- PrismReel-specific compatibility notes
...
**Purpose:** Make model support in PrismReel data-driven and enforce a single source of truth for runtime behavior.

This is the most important new layer.

`model_catalog` should answer these questions:

- Which models does PrismReel know about?
...
If raw docs answer "what the vendor says", `model_catalog` answers "what PrismReel supports".
...
**Repository location:** inside the main PrismReel repo.
...
## How This Fits The Current PrismReel Codebase
...
- Introduce a valid `model_catalog` format inside PrismReel.
...
### New files in the PrismReel repo
