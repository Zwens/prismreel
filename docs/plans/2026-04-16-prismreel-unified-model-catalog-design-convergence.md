# PrismReel Unified Model Catalog Design Convergence

> Date: 2026-04-16  
> Status: Pre-implementation design convergence  
> Scope: PrismReel-first decision document  
> Purpose: Resolve the remaining design questions before implementation, while explicitly minimizing migration risk for the current PrismReel codebase.
...
But before implementation, PrismReel still needs one more thing:

> a practical convergence decision memo that answers “what do we actually do first in PrismReel, and how do we avoid a high-cost migration?”
...
For PrismReel, **migration means internal schema evolution with compatibility preservation**, not:
...
## 3. Final Position For PrismReel

PrismReel **can** adapt to the unified model-catalog direction, and it can do so with acceptable risk **if and only if** we keep these rules:

### Rule 1

**Do not change PrismReel frontend consumer shape in phase 1.**
...
In phase 1, runtime overlay fields may exist in the source schema, but PrismReel does not need to consume them everywhere yet.
...
This section answers the major open questions from Spec v0.1 specifically for PrismReel-first implementation.

---

### Decision A: Canonical backend naming in shared core

**Decision:** Use `dashscope` as the canonical backend name in PrismReel’s source schema.

### Why

- PrismReel already uses `dashscope` in:
  - provider routing
  - backend env keys
  - current catalog source
- The app domain is “Alibaba DashScope / 百炼 as product-facing backend”
- Renaming PrismReel internals to `bailian` would create churn without real value
...
- do **not** force PrismReel to rename its current backend key now

### Implication

For PrismReel implementation:
...
**Decision:** In PrismReel, `product` overlay should live primarily at the **mode level**.

### Why

Because PrismReel product behavior is mode-sensitive:
...
**Decision:** Keep `routing_prefixes` in PrismReel phase 1 as a compatibility field.

### Why

Today PrismReel provider resolution still benefits from family-prefix matching.
...
In PrismReel today, transport semantics are mostly provider-family concerns:
...
### Decision F: What should `defaults.model_settings` point to in PrismReel

**Decision:** In phase 1, PrismReel-generated artifacts should continue to expose **legacy-compatible flat IDs** in `defaults.model_settings`.
...
## 6. What PrismReel Should Implement First
...
## 7. What PrismReel Should Explicitly Avoid In Phase 1
...
- overlay separation
- `dashscope` remains canonical backend key in PrismReel
...
> In PrismReel, implement unified catalog convergence as a compatibility-preserving source-schema upgrade first, and postpone consumer rewrites until the generated contract has proven stable.
