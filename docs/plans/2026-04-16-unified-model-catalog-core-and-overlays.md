...
>
> 1. PrismReel repo-native `model_catalog`
...
- **PrismReel** uses `model_catalog` to drive:
...
2. A **PrismReel product overlay** for UI and product-specific behavior
...
- PrismReel is stronger at **product/UI consumption modeling**
...
### 5.1 PrismReel optimizes for product and UI consumption

PrismReel catalog currently excels at fields like:
...
PrismReel mostly models video capability like this:
...
This layer should be reusable by both PrismReel and video-generator.

### Layer B — Product Overlay

This layer stores PrismReel-only concerns:
...
### 9.2 PrismReel product overlay fields

These are legitimate PrismReel-only fields:
...
### 10.1 PrismReel → unified core

| PrismReel field | Unified target | Notes |
|---|---|---|
| `family` | `family` | Keep |
...
| `defaults.backend_priority` | root `defaults.backend_priority` | Strong candidate to adopt in PrismReel too |
| `docs.integration_doc_ids` | `docs.integration_doc_ids` | Keep |
| `docs.vendor_doc_paths` | `docs.vendor_doc_paths` | Strongly worth adopting in PrismReel |
...
## 11. What PrismReel Should Borrow First

If implementation starts incrementally, PrismReel should borrow these parts first, in this order:
...
This lets PrismReel gradually move toward catalog-driven runtime execution.
...
## 12. What video-generator Should Borrow From PrismReel

This should also be explicit, because convergence is not one-way.

### 12.1 Product-facing visibility concepts

If video-generator ever powers an interactive UI, PrismReel’s current notions are already useful:
...
PrismReel already treats stale saved model IDs as a product reality.
...
### Step 2 — Upgrade PrismReel schema shape without changing behavior
...
### Step 3 — Add runtime overlay fields to PrismReel catalog
...
If we try to force identical runtime behavior into both projects too early, we may damage the product-specific strengths of PrismReel or the CLI strengths of video-generator.
...
If convergence focuses only on runtime execution, PrismReel may lose valuable UI-specific metadata.
...
5. validation rules
6. phased implementation plan for PrismReel
...
3. preserve PrismReel’s product metadata
4. preserve video-generator’s runtime metadata
5. converge schema before converging code

### In one sentence

> Build one shared catalog language, then let PrismReel and video-generator speak different dialects of it through overlays.
