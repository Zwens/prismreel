...
>
> 1. **PrismReel** as a product/UI system
...
- PrismReel catalog is strong at:
  - frontend visibility
  - defaults
...
- removal of existing PrismReel consumer behavior in the same iteration
...
- PrismReel may ignore `runtime.vendor.submit_url`
...
- root-level product defaults for PrismReel model settings
...
- existing saved PrismReel projects
...
- Product-level defaults for consumers like PrismReel
...
PrismReel frontend currently uses camelCase-like keys in some places:
...
Runtime overlay is for execution engines like video-generator and future PrismReel runtime consumers.
...
Product overlay is for UI and product-facing systems like PrismReel.
...
### 15.2 PrismReel compatibility

PrismReel currently expects:
...
### Phase 1 — Upgrade PrismReel authoring schema
...
### Phase 3 — Add runtime overlay fields to PrismReel

Goal:

- allow PrismReel to gradually become runtime-profile aware
...
2. do not break PrismReel frontend consumer contracts yet
...
> Define one canonical model catalog grammar, then let PrismReel and video-generator consume different normalized views of the same truth.
