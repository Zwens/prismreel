...
PrismReel Studio 的视觉重构分为两条完全对立的方向，都基于同一个前提——**dark-first · 内容为王 · creator cockpit**。
...
- Branding：顶部仅保留 logo 图标，hover 浮出 "PrismReel Studio" 全称
...
| 资产 | 建议去向 | 状态 |
|------|---------|------|
| **PRISMREEL 字标** | 导轨顶部仅 logo 图标 + hover 浮出全称（已实现） | ✅ |
