...
Burns an evenly-spaced NxN grid (black 1px lines) permanently into an
uploaded image, to help downstream AI models read proportions/composition
(e.g. character pose or face-swap reference photos). This is opt-in per
upload (grid_size=0 leaves the image untouched) and, once applied, is not
reversible — the caller is responsible for keeping their own original if
they need it later.
"""
...
def apply_grid_overlay(image_bytes: bytes, ext: str, grid_size: int) -> bytes:
    ...
    function without a separate branch.
    """
    if grid_size == 0:
        return image_bytes
